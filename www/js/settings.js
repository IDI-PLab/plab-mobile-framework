...
plab.settingsController = {
	...
	// Setting change listeners. DO NOT MANUALLY CHANGE. use add or remove listeners
	changeListeners : {},
	// Visibility state of setting menu
	visible : false,
	...
	// Add a new item to settings menu
	...
	// Get or set a specific setting value
	getSettingValue : function(id) {
		return window.localStorage.getItem(id);
	},
	setSettingValue : function(id, val) {
		// Check against old value
		var oldVal = plab.settingsController.getSettingValue(id);
		if (val !== oldVal) {
			// Value has changed, store it and fire change events
			window.localStorage.setItem(id,val);
			if (typeof plab.settingsController.changeListeners[id] !== "undefined") {
				for (var i = 0; i < plab.settingsController.changeListeners[id].length; i++) {
					var l = plab.settingsController.changeListeners[id][i];
	...
	// Add or remove listeners to setting change
	addChangeListener : function(id, listener) {
		if (typeof plab.settingsController.changeListeners[id] === "undefined") {
			plab.settingsController.changeListeners[id] = [];
		}
		plab.settingsController.changeListeners[id][plab.settingsController.changeListeners[id].length] = listener;
	},
	removeChangeListener : function(id, listener) {
		if (typeof plab.settingsController.changeListeners[id] === "undefined") { return; }
		var i = plab.settingsController.changeListeners[id].indexOf(listener);
		if (i > -1) {
			plab.settingsController.changeListeners[id].splice(i, 1);
		}
	},
	
	// Internal functions that are used for menu setting creation
	...
	createSingleSelectNode : function(item) {
		...
		// MVC pattern: add listener to setting controller
		plab.settingsController.addChangeListener(item.id, function(id, oldVal, newVal) {
